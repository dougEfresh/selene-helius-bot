...
async fn handle_hook(
  transactions: Vec<EnhancedTransaction>,
  bot: Arc<SeleneBot>,
...
async fn serve(args: command::ServeArgs) -> color_eyre::Result<()> {
  ...
  let metrics_route =
    warp::path("metrics").and_then(metrics::handler).with(warp::trace::named("metrics"));

  let goaway = warp::any()
    .map(|| warp::reply::with_status("go away", StatusCode::FORBIDDEN))
    .with(warp::trace::named("goaway"));
